...
for i = 1:nstep
    ...
    x(i+1) = x(i) + vel(i) * dir_x * dt;
    y(i+1) = y(i) + vel(i) * dir_y * dt;
    ...
    contour(u_total.')
    hold on
    plot(x,y,'-',x(i),y(i),'r>',q_goal(1),q_goal(2),'r>');
    hold off
    xlim([0 100])
    ylim([0 100])
    pause(0.01)
